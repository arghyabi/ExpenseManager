...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'add') {
        ...
        $stmt = $db->prepare(
            "INSERT INTO transactions (date, type, amount, category_id, account_id, note, payment_mode)
             VALUES (?, ?, ?, ?, ?, ?, 'cash')"
        );
        ...
        $stmt->bindValue(5, $account, SQLITE3_INTEGER);
        $stmt->bindValue(6, $note, SQLITE3_TEXT);
        $stmt->execute();
    }
    elseif ($action === 'edit' && $tx_id > 0) {
        $date = isset($_POST['date']) ? $_POST['date'] : '';
        $type = isset($_POST['type']) ? $_POST['type'] : '';
        $amount = isset($_POST['amount']) ? floatval($_POST['amount']) : 0;
        $category = isset($_POST['category']) ? intval($_POST['category']) : 0;
        ...
        $stmt = $db->prepare(
            "UPDATE transactions
             SET date=?, type=?, amount=?, category_id=?, account_id=?, note=?
             WHERE id=?"
        );
        $stmt->bindValue(1, $date, SQLITE3_TEXT);
        $stmt->bindValue(2, $type, SQLITE3_TEXT);
        ...
        $stmt->bindValue(5, $account, SQLITE3_INTEGER);
        $stmt->bindValue(6, $note, SQLITE3_TEXT);
        $stmt->bindValue(7, $tx_id, SQLITE3_INTEGER);
        $stmt->execute();
    }
